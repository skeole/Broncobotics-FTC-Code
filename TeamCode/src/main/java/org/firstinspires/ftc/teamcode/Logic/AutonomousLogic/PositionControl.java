...
public class PositionControl extends RobotHardware {
    ...
    public PositionControl(StandardTrackingWheelLocalizer l, double x_, double y_, double angle_) {
        position_tracker = l;
    ...
    public void drive(double turning_factor, double speed) {
        for (int i = 0; i < 4; i++) {
            wheel_list[i].setPower(speed * turning_factor * ((i > 1) ? -1 : 1));
        }
    }
    ...
    public void drive(double distance_factor, double offset, double speed) {
        ...
        for (int i = 0; i < 4; i++) {
            wheel_list[i].setPower(power[i] / maximum * speed);
        }
    }
